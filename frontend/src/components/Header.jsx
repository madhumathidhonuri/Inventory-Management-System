      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="relative" ref={userDropdownRef}>
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 text-xs font-semibold text-slate-800 flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs"
          >
            <div className="w-5 h-5 rounded-full bg-slate-800 text-white flex items-center justify-center text-[10px] font-bold">
              {(user?.name || 'U').charAt(0).toUpperCase()}
            </div>
            <span className="hidden sm:inline font-bold text-slate-800">{user?.name || 'Account'}</span>
            <ChevronDown className="w-3 h-3 text-slate-400" />
          </button>

          {showUserDropdown && (
            <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-2xl p-2.5 z-50 animate-in fade-in-50 duration-150 space-y-2">
              <div className="px-3 py-2 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-2.5">
                ...
                <div className="overflow-hidden">
                  <div className="text-xs font-bold text-slate-900 truncate">{user?.name || 'Account'}</div>
