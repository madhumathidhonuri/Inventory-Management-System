      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="relative" ref={userDropdownRef}>
          <button
            ...
            title={`${user?.name || 'User Profile'} (${roleInfo.label})`}
            className="w-8 h-8 rounded-xl bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 flex items-center justify-center transition-all cursor-pointer shadow-xs"
          >
            <User className="w-4 h-4" />
          </button>

          {showUserDropdown && (
            <div className="absolute right-0 mt-2 w-60 bg-white border border-slate-200 rounded-xl shadow-xl p-2 z-50 animate-in fade-in-50 duration-150">
              <div className="px-3 py-2 bg-slate-50 rounded-lg border border-slate-100 mb-1.5">
                <div className="text-xs font-bold text-slate-800">{user?.name || roleInfo.label}</div>
                <div className="text-[11px] text-slate-500 truncate">{user?.email || 'Logged In'}</div>
                <div className="mt-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-purple-100 text-purple-800 border border-purple-200">
                  {roleInfo.label}
                </div>
              </div>
