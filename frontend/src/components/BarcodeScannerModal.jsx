...
export default function BarcodeScannerModal({
  ...
  const loadDeviceCatalog = async () => {
    try {
      ...
      console.warn('Could not load device catalog:', e);
    }
  };
  ...
  const detectImeisAsync = async (imeiList) => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        ...
        const detectedName = res.detected_type_name || (res.data[0] && (res.data[0].device_name || res.data[0].device_type_name));
        const detectedTypeId = res.detected_type_id || (res.data[0] && res.data[0].device_type_id);
        ...
        if (deviceTypes && deviceTypes.length > 0) {
          let matched = null;
          if (detectedTypeId) {
            matched = deviceTypes.find(dt => dt.id === Number(detectedTypeId));
          }
          if (!matched && detectedName) {
            const cleanName = detectedName.toLowerCase().replace(/[^a-z0-9]/g, '');
            matched = deviceTypes.find(dt => {
              const dtClean = dt.name.toLowerCase().replace(/[^a-z0-9]/g, '');
              return dtClean === cleanName || dtClean.includes(cleanName) || cleanName.includes(dtClean);
            });
          }
          if (matched) {
      ...
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-150">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex flex-col h-[320px]">
            ...
            {lastScannedImei && (
              <div className="mb-2 p-2 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between gap-2 text-emerald-800 text-xs animate-in fade-in-50">
                <div className="flex items-center gap-1.5 truncate">
                  <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                  <span className="text-[11px]">Scanned: <strong className="font-mono font-bold">{lastScannedImei}</strong></span>
                </div>
                {(detectedMap[lastScannedImei]?.device_name || lastDetectedModel) && (
                  <span className="bg-emerald-600 text-white px-2 py-0.5 rounded-lg text-[10px] font-bold shadow-2xs shrink-0 flex items-center gap-1">
                    <Sparkles className="w-3 h-3" /> {detectedMap[lastScannedImei]?.device_name || lastDetectedModel}
                  </span>
                )}
              </div>
            )}

            {/* Scanned Items List with Clean Multi-Tag Row Layout */}
            <div className="flex-1 overflow-y-auto space-y-2 pr-1">
              {scannedImeis.length === 0 ? (
                ...
                scannedImeis.map((imei, idx) => {
                  ...
                  const devName = det?.device_name || det?.device_type_name || lastDetectedModel || 'GPS Tracker';
                  const stockPlace = det?.stock_place || (det?.exists ? 'Central Warehouse' : 'New Device');
                  return (
                    <div
                      key={idx}
                      className="p-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-800 shadow-2xs hover:border-blue-300 transition-colors space-y-1.5"
                    >
                      {/* Top Header Row: Index + Full IMEI + Delete Button */}
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <strong className="font-mono text-blue-700 text-xs select-all tracking-wide">
                            {imei}
                          </strong>
                        </div>
                        <button
                      ...
                      {/* Bottom Info Tags (Model Name, Stock Place, Status) */}
