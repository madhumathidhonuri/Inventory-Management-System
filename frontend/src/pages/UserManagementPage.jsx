...
import { 
  ...
  Sparkles, Sliders, Layers, DollarSign, Car, Building, Boxes, Target
} from 'lucide-react';
import { fetchUsers, createUser, updateUser, deleteUser, fetchDeviceTypes, factoryResetSystem } from '../services/api';
...
export default function UserManagementPage() {
  ...
  // Factory Reset All Data & Roles State
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [usersRes, devTypesRes] = await Promise.all([
        fetchUsers(),
        fetchDeviceTypes().catch(() => ({ success: false }))
      ]);
      if (usersRes.success) {
        setUsers(usersRes.data || []);
      }
      if (devTypesRes.success) {
  ...
  const handleExecuteFactoryReset = async () => {
    ...
    try {
      const res = await factoryResetSystem();
      alert(`✅ ${res.message || 'System completely reset to factory fresh state!'}`);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
            title="Factory Reset: Wipe all devices, uploads, and non-admin team roles"
          >
            <Trash2 className="w-4 h-4 text-red-600" /> Reset All Data & Roles
          </button>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {users.map((u) => {
                  ...
                  const devTargets = typeof u.device_targets === 'object' && u.device_targets !== null ? u.device_targets : {};
                  const devTargetEntries = Object.entries(devTargets).filter(([_, v]) => Number(v) > 0);
      ...
      {/* Factory Reset All Data & Roles Modal */}
      {isResetModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="text-center space-y-1">
              <h3 className="text-base font-bold text-slate-900">Factory Reset Everything?</h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                This will permanently delete <strong>all inventory devices, Excel upload batches, installations, customers, dispatches, and all non-admin user roles</strong>.
              </p>
              <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 p-2.5 rounded-xl font-medium mt-2">
                ⚠️ Your primary Super Admin account (DHONURI MADHUMATHI / 9999999999) will remain safely active so you can log in and upload fresh data.
              </p>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 text-center font-mono font-bold text-sm text-red-600 tracking-wider focus:outline-none focus:border-red-500"
              />
            </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-40 text-white text-xs font-bold rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5"
              >
                {resetLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                Wipe Everything Fresh
              </button>
            </div>
          </div>
        </div>
